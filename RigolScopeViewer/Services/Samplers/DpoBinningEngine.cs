...
public class DpoBinningEngine : IResampler<ColumnStats>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Орендуємо масиви з пулу замість створення нових
        var minPool = ArrayPool<float>.Shared.Rent(numBins);
        var maxPool = ArrayPool<float>.Shared.Rent(numBins);
        var integralPool = ArrayPool<double>.Shared.Rent(numBins);
        var timePool = ArrayPool<double>.Shared.Rent(numBins);

        try
        {
            // Робимо зрізи точної довжини (бо пул може видати масив трохи більшого розміру)
            Span<float> binMinVoltages = minPool.AsSpan(0, numBins);
            Span<float> binMaxVoltages = maxPool.AsSpan(0, numBins);
            Span<double> voltageTimeIntegral = integralPool.AsSpan(0, numBins);
            Span<double> accumulatedTime = timePool.AsSpan(0, numBins);

            // Ініціалізуємо екстремуми екстремальними значеннями для подальшого пошуку Min/Max
            binMinVoltages.Fill(float.MaxValue);
            binMaxVoltages.Fill(float.MinValue);
        ...
        finally
        {
            ...
            // Повертаємо масиви назад у пул
            ArrayPool<float>.Shared.Return(minPool);
            ArrayPool<float>.Shared.Return(maxPool);
            ArrayPool<double>.Shared.Return(integralPool);
            ArrayPool<double>.Shared.Return(timePool);
        }
    }
}
